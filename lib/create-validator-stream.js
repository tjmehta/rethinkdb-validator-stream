...
function createValidatorStream (opts, ignoreHandshake) {
  var chunkAndValidate = pipeline(
    createQueryChunker(ignoreHandshake),
    _createValidatorStream(opts, ignoreHandshake)
  )
  // merge streams into one
  return chunkAndValidate
}

function _createValidatorStream (opts, ignoreHandshake) {
  opts = opts || {}
  ...
  var handshakeComplete = ignoreHandshake
